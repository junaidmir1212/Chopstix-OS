...
import { faHome, faRobot, faGraduationCap, faTrash, faSignOutAlt, faBars, faTimes, faUserCircle, faLock, faBuilding, faIdBadge, faKey } from '@fortawesome/free-solid-svg-icons';
          ...
          <>
            {!isLoggedIn ? (
              ...
              <>
                {/* 1. PROFILE MODAL */}
                {showProfileModal && (
                  <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10000, backdropFilter: 'blur(8px)' }}>
                    <div style={{ backgroundColor: 'white', width: '90%', maxWidth: '420px', borderRadius: '28px', overflow: 'hidden', boxShadow: '0 25px 50px rgba(0,0,0,0.3)' }}>
                      <div style={{ backgroundColor: theme.sidebarBg, padding: '40px 20px', textAlign: 'center', color: 'white', position: 'relative' }}>
                        <button onClick={() => setShowProfileModal(false)} style={{ position: 'absolute', top: '20px', right: '20px', background: 'rgba(255,255,255,0.1)', border: 'none', color: 'white', width: '32px', height: '32px', borderRadius: '50%', cursor: 'pointer' }}><FontAwesomeIcon icon={faTimes} /></button>
                        <div style={{ width: '90px', height: '90px', borderRadius: '50%', backgroundColor: theme.primaryRed, margin: '0 auto 15px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '32px', fontWeight: '900', border: '4px solid rgba(255,255,255,0.1)' }}>{getInitials(userName)}</div>
                        <h3 style={{ margin: 0, fontSize: '22px' }}>{userName}</h3>
                        <p style={{ margin: '5px 0 0', opacity: 0.8, fontSize: '14px', textTransform: 'uppercase', letterSpacing: '1px' }}>{userRole.replace('_', ' ')}</p>
                      </div>
                      <div style={{ padding: '30px' }}>
                        <div style={{ display: 'grid', gap: '20px', marginBottom: '30px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                            <div style={{ width: '40px', height: '40px', borderRadius: '10px', backgroundColor: '#F3F4F6', display: 'flex', alignItems: 'center', justifyContent: 'center', color: theme.primaryRed }}><FontAwesomeIcon icon={faIdBadge} /></div>
                            <div><div style={{ fontSize: '11px', fontWeight: '700', color: theme.textMuted }}>POSITION</div><div style={{ fontWeight: '700', color: theme.textDark }}>{userRole === "area_manager" ? "Area Director" : "Branch Manager"}</div></div>
                          </div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                            <div style={{ width: '40px', height: '40px', borderRadius: '10px', backgroundColor: '#F3F4F6', display: 'flex', alignItems: 'center', justifyContent: 'center', color: theme.primaryRed }}><FontAwesomeIcon icon={faBuilding} /></div>
                            <div><div style={{ fontSize: '11px', fontWeight: '700', color: theme.textMuted }}>BRANCH / AREA</div><div style={{ fontWeight: '700', color: theme.textDark }}>{selectedBranch}</div></div>
                          </div>
                        </div>
                        <div style={{ padding: '20px', backgroundColor: '#F9FAFB', borderRadius: '16px', border: `1px solid ${theme.border}` }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '15px', color: theme.textDark }}><FontAwesomeIcon icon={faKey} /> <span style={{fontWeight: '700', fontSize: '14px'}}>Security</span></div>
                          <input type="password" placeholder="Enter New Password" style={{ width: '100%', padding: '12px', borderRadius: '10px', border: `1px solid ${theme.border}`, marginBottom: '12px', boxSizing: 'border-box', outline: 'none' }} />
                          <button style={{ width: '100%', padding: '12px', backgroundColor: theme.primaryRed, color: 'white', border: 'none', borderRadius: '10px', fontWeight: '800', cursor: 'pointer' }}>Change Password</button>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="main-content" style={{ flex: 1, transition: 'margin 0.3s ease', minWidth: 0 }}>
                  <header style={{ height: '70px', backgroundColor: theme.white, borderBottom: `1px solid ${theme.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 40px', position: 'relative' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }} ref={dropdownRef}>
                      ...
                      {isProfileOpen && (
                        <div className="profile-dropdown" style={{ position: 'absolute', top: '65px', right: '40px', width: '190px', backgroundColor: 'white', borderRadius: '16px', boxShadow: '0 10px 30px rgba(0,0,0,0.1)', border: `1px solid ${theme.border}`, zIndex: 1100, overflow: 'hidden' }}>
                          <button onClick={() => { setShowProfileModal(true); setIsProfileOpen(false); }} style={{ width: '100%', padding: '14px 18px', textAlign: 'left', border: 'none', background: 'none', cursor: 'pointer', fontSize: '14px', fontWeight: '700', color: theme.textDark, display: 'flex', alignItems: 'center', gap: '12px' }}><FontAwesomeIcon icon={faUserCircle} /> View Profile</button>
                          <button onClick={handleLogout} style={{ width: '100%', padding: '14px 18px', textAlign: 'left', border: 'none', background: 'none', cursor: 'pointer', fontSize: '14px', fontWeight: '700', color: theme.danger, display: 'flex', alignItems: 'center', gap: '12px', borderTop: `1px solid ${theme.border}` }}><FontAwesomeIcon icon={faSignOutAlt} /> Log Out</button>
                        </div>
                      )}
                    </div>
                  </header>
